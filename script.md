...
So what does 'easy APIs' actually look like? Well, we all know the Knuth quote — 'premature optimization is the root of all evil.' I think that applies to API design just as much as it applies to code. And I'm guilty of this too btw. We reach for traits when an enum would do. We add lifetime parameters to avoid a clone. We design for flexibility nobody asked for. And we end up with APIs that are powerful but painful to use. So when I started building for the higher level, I wanted to take a different approach. Prioritize productivity. Make it easy to use first, optimize later.
