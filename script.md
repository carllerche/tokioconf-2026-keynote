...
This is the earliest version of the Future trait I could find, and you can already see the shape of today's future trait. Now, remember, up until now, every implementation required an allocated callback and channel at each step. This trait was a breakthrough — instead of callbacks, the runtime polls the future, asking "are you done yet?" If it is, great. But what if it isn't? This is async — you can't just block the thread and wait. Something has to tell the runtime when to come back and poll again. That's what the second method, `schedule`, is for. The runtime calls it with a task handle, and the future is responsible for using that handle to notify the runtime when it's ready. It worked, but in practice, `poll` and `schedule` ended up being almost identical code. You were writing everything twice.

# Tokio

And this gets us to the original Tokio announcement in August 2016. That duplication kept bugging me, and I think the solution I came up with is probably the one bit of invention I actually provided to Rust's async story, so I'm going to revel in it for a bit.
...
This is the read function for the TCP socket type in the very first version of Tokio — I dug up the code and copy/pasted it. Look at the signature — it's identical to a regular blocking read, and there is no separate `schedule` method. What I realized was: if the runtime is polling a socket for data and it isn't ready, we should assume the runtime wants to be notified when the socket becomes ready. That's it. The poll itself already tells you everything you need to know. So `schedule` just... went away. I also thought it was clever to match the blocking read signature exactly — the idea was that existing implementations of the Read and Write traits would just work with Tokio. In practice, that didn't work out so well, so it didn't stick. But that core idea — that polling implies interest — that did stick. It's the foundation of Rust async IO today.

So yeah, that was Tokio back then... very different, well before async/await. And to be honest, using it was not great. But people were still building with it.

# Pre-async/await

In 2017, I joined Buoyant to work on Linkerd, a service mesh. They had originally built it in Scala, but a service mesh runs as a sidecar proxy alongside every service — so the memory footprint has to be tiny. They needed to get it under 10 megabytes of RSS. They tried every trick in the book to reduce the JVM's memory footprint, but it just wasn't happening. A rewrite was the last resort. So it was Rust or C, and Rust won because of the safety guarantees. But if they could have stayed on the JVM, they probably would have. And honestly, I don't blame them. We did successfully rewrite Linkerd in Rust, but without async/await, every single future was written by hand. Enums for every state, match statements for every transition. In a codebase that size... yeah. It worked, but nobody was choosing Rust for networking because they wanted to — they were choosing it because they had to. It was fast, it was reliable, but the productivity story wasn't there yet. And without fixing that, I wasn't sure how much further we'd go. Because, fundamentally, technology adoption is about productivity: how fast can I use a programming language, a library, a tool, to accomplish my goal.
...
And yes, IDE support has historically been a pain point with macros. But it's improving — rust-analyzer can already provide autocomplete through macros when they're implemented correctly. It's not perfect yet, but it's getting there. The black box concern is real, but it can be mitigated. For example, that `create!` macro in Toasty? It's a light transformation — it expands to an equivalent builder API. You could write it by hand, the macro just saves you the boilerplate. Keep the transformations simple, use macros where they give you the biggest leverage, and the tradeoff is absolutely worth it.

But I think we can go further than just rebuilding what other languages have. We can build libraries that take advantage of Rust's unique strengths, and be better because of it. And honestly, this is a lesson I had to learn myself. You saw Eventual earlier — that was me trying to copy the callback pattern from other languages, and it didn't work out in Rust. But when we leaned into what Rust is actually good at, we got Tokio. And it was so much better. And every time I start a new library, I go through the same thing. I look at what other languages do and hope I can just copy it — because that would be a lot easier than coming up with something new. But it never quite works. What works is leaning into what Rust is actually good at. And that's why, when I started working on Toasty, I ended up rethinking ORMs from the ground up.

Now, I know what some of you are thinking. ORMs can trigger a strong response. Many of you have been burned before.  "I don't like ORMs. I'd rather just write SQL. ORMs are too complex." And you aren't wrong to be skeptical. But think about what SQL databases do under the hood — query planners, optimizers, tons of complexity. And we're all fine with that. The difference isn't the complexity — it's whether the abstraction actually holds up. SQL engines are smart enough to handle the complexity they take on. ORMs just aren't. So you hit a wall, there's no escape hatch, and you throw it all out and go back to raw SQL. But that's throwing the baby out with the bathwater.

# Application data is graph-based

So when I started Toasty, I took the lesson I keep learning and asked — instead of copying what other languages have done, can we lean into Rust to do something better? The fundamental challenge is that application data is graph-based — a user has many todos, a todo belongs to a category — but SQL is relational. And what existing ORMs do is take a query written against that graph model and try to directly translate it to SQL. That works for simple cases, but the two models don't map cleanly onto each other, so the translation breaks down fast. That's the wall everyone hits.
...
Now, to actually pull this off, Toasty needs something that, as far as I know, no ORM has tried before — a full application-level query engine. One that can actually understand, validate, plan, and translate queries across both levels. And this is where it comes full circle to Rust. Years ago, I was actually on the Ruby on Rails core team — Rails is a web framework, and ActiveRecord is its ORM — and, back then, we talked about giving ActiveRecord a deeper understanding of the application-level data model. But Ruby was way too slow, so it wasn't really an option. Rust really is the only language where putting a query engine into an ORM makes sense because it is both expressive enough for high-level abstractions and performant enough to do the work.

And this is what excites me most about Rust. It's a language where you can build a query engine and an ORM in the same codebase. Think about what that means more broadly. Rust is a language where you can do work that's traditionally considered low-level — font rendering, layout engines, game engines, GPU programming — and high-level work like ORMs, web frameworks, developer tooling — in the same language. The ability to blend both into one is a superpower. And honestly, I don't think we've even scratched the surface of what that makes possible.
