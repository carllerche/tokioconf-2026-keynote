...
This is the read function for the TCP socket type in the very first version of Tokio — I dug up the code and copy/pasted it. Look at the signature — it's identical to a regular blocking read, and there is no separate `schedule` method. What I realized was: if the runtime is polling a socket for data and it isn't ready, we should assume the runtime wants to be notified when the socket becomes ready. That's it. The poll itself already tells you everything you need to know. So `schedule` just... went away. Now, some of the other details of this approach didn't stick, and probably for good reason. But that core idea — that polling implies interest — that did stick. It's the foundation of Rust async IO today.
